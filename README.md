...
<p align="center"><strong>More work per agent, per dollar.</strong></p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-543_passing-brightgreen?style=flat" alt="543 tests">
...
| **Providers & cache** | Executable provider adapters, provider-native caching, and byte-stable public cache planning. |
...
| **Adapters** | Separate exact-pinned packages for Pi, Claude Agent SDK, Vercel AI SDK, Eve, and Mastra. Shared manifest/registry contract fails closed on uncertified capability claims. |
| **Coding agent** | Separate `@caveman-ai/coding-agent` product and `caveman-code` binary over the core runtime. |
...
Add only framework adapter needed:
...
Monorepo boundaries and dependency rules: [docs/MONOREPO.md](./docs/MONOREPO.md).
...
```bash
npm ci                              # install all workspaces from root lock
...
`npm test` checks adapter manifests/registry, coding CLI boundary, generated
provider catalog, runtime/compiler suites, deterministic replay artifact,
initializer, and example agent. macOS restricted-tool tests require working
`sandbox-exec` and loopback access.

## Packages

- `packages/agent` — `@caveman-ai/agent` v0.2 compiler/runtime.
- `packages/adapter-kit` — framework-neutral manifest, capability, registry,
  and conformance authorization contract.
- `packages/adapters/*` — one package per exact-pinned upstream framework.
...
- `packages/pebble-protocol` — frozen Apache-2.0 wire and session contract;
  proprietary implementation lives elsewhere.
...
Apache-2.0 for the entire repository. See [LICENSING.md](./LICENSING.md).
Anthropic Claude Agent SDK dependency remains governed by Anthropic terms; see
framework README and sandbox threat model for exact boundary.
