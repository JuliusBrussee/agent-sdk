/**
 * Canonical vectors for llm_score, llm_category, llm_pairwise, and
 * llm_answer_match. Two things are asserted per vector:
 *   1. the VERDICT (`expected_passed`), and
 *   2. the exact PROMPT TEXT(S) that reached the wire (`expected_prompts`) — the
 *      pinned templates are a byte contract, so a reflowed or re-punctuated
 *      prompt fails even when the verdict agrees.
 ...
 * include. This is deliberately a containment check rather than equality. It
 * exists for reason-size caps: the fragment is the
 ...
 * expected_prompts, expected_reason_contains?}. `options` excludes gateway_url;
 * test injects a fake URL plus transport. Gateway URL never reaches a prompt.
...
const vectors = JSON.parse(readFileSync(join(here, "judge-vectors.json"), "utf8"));
