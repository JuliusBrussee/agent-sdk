...
test("exact_match normalises case, whitespace, and structured key order", async () => {
  assert.equal((await grade({ type: "exact_match", expected: "Paris" }, " paris ")).passed, true);
...
test("regex worker deadline stops catastrophic adjacent quantifiers", async () => {
  ...
  assert.ok(Date.now() - started < 500, "regex worker deadline did not settle promptly");
});
...
test("json_schema rejects unsupported constraints instead of weakening the gate", async () => {
  const result = await grade({ type: "json_schema", schema: { type: "number", minimum: 10 } }, 1);
  assert.equal(result.passed, false);
  assert.match(result.reason, /unsupported schema keywords.*minimum/);
});

test("json_schema validates nested definitions even when candidate property is absent", async () => {
  const result = await grade(
    { type: "json_schema", schema: { type: "object", properties: { optional: { pattern: "^safe$" } } } },
    {},
  );
  assert.equal(result.passed, false);
  assert.match(result.reason, /unsupported schema keywords.*pattern/);
});

test("json_schema enum comparison is structural and rejects non-finite JSON numbers", async () => {
  assert.equal(
    (await grade({ type: "json_schema", schema: { enum: [{ a: 1, b: 2 }] } }, { b: 2, a: 1 })).passed,
    true,
  );
  assert.equal((await grade({ type: "json_schema", schema: { type: "number" } }, Number.NaN)).passed, false);
});

test("malformed runtime graders return a failed verdict instead of throwing", async () => {
  for (const grader of [null, [], {}, { type: "contains" }, { type: "json_path_assertion" }]) {
    const result = await grade(grader, "x");
    assert.equal(result.passed, false);
  }
  assert.equal((await grade({ type: "exact_match" }, undefined)).passed, false);
});
...
test("json_path_assertion equals is key-order-insensitive", async () => {
  assert.equal(
...
test("tool_argument_assertion equals is key-order-insensitive", async () => {
...
// ─── cross-runtime pinning ───────────────────────────────────────────────────
// These tests pin runtime edge cases and bounded-input behavior.
...
test("A2 not_regex normalises CR and one trailing newline before matching", async () => {
  ...
  // Runtimes differ on `$` before a final newline. Pinned normalization covers one
  // trailing newline; two remain candidate content and intentionally do not match.
  assert.equal((await grade({ type: "not_regex", pattern: "ok$" }, "ok\n\n")).passed, true);
...
// ─── judge parse/serialisation contract ──────────────────────────────────────
// These cases prevent runtime regex/JSON defaults from producing divergent
// verdicts for one judge reply.
