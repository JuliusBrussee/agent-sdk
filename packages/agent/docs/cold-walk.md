...
| 6 | `npm run build` with evals unapproved | static checks run first, then `build status: needs_eval` with next action naming the approval flip; exit 1 | VERIFIED-DRY |
...
| 7 | magic moment A, the save: add the README's exact volatile snippet (`today: () => \`Today is ${new Date().toDateString()}\``) to `agent.ts`, `npm run build` | build FAILS with the golden voice (`goldens/failures/volatile-prefix.txt`): plain-words sentence, mechanism, fix, closing line; fires with evals still unapproved; wire code appears only under `--verbose` | VERIFIED-DRY — fired byte-shape-identical to the golden (live fills location `agent.ts` and fix path `tools/get_today.ts`; the golden pins the fixture's `agent.ts:8` / `get_date.ts`) |
| 8 | remove the snippet, `npm run build` | back to `needs_eval` — the save leaves no residue | VERIFIED-DRY |
| 9 | flip `APPROVED = true`, `npm run build` | **BLOCKED — see finding below.** The eval gate engages and fail-closes with `capability_refused: cave_compiler_tool_effect_coverage_unavailable` before any provider call | VERIFIED-DRY (the refusal); the intended lock is not reachable |
...
Also fixed during this walk (was blocking step 2): the template's eval file
