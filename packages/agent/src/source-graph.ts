...
function resolvePackageJSON(packageName: string, importer: string): string {
  const packageJSONPath = findPackageJSONCompat(packageName, pathToFileURL(importer));
...
async function collectPackageClosure(
  ...
  for (const dependency of [...new Set([...required, ...optional])].sort()) {
    try {
      const childPackageJSON = findPackageJSONCompat(dependency, pathToFileURL(packageJSONPath));
