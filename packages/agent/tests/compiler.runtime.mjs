...
function transformedPlan(id) {
  return {
    ...
    recovery: { namespace: "test", tools: ["cave_retrieve"] },
  };
}
...
function evidence({
  ...
  const inputTokens = Math.max(
    0,
    Math.round(cost * 1_000_000 - cacheRead * 0.1 - cacheWrite * 1.25),
  );
  return {
    terminal: true,
    provider: "anthropic",
    ...
    graders: [{ type: graderType, passed }],
    latency_ms: latency,
...
test("runner evidence is strict, total, and bound to configured graders", async () => {
  const baseline = plan("baseline");
  const malformed = [
    { ...evidence(), terminal: "yes" },
    { ...evidence(), graders: undefined },
    { ...evidence(), graders: [{ type: "contains", passed: "yes" }] },
    { ...evidence(), latency_ms: Number.NaN },
    { ...evidence(), error: 0 },
    { ...evidence(), provider_visible_tokens: 1 },
    { ...evidence(), recovery_resolved: "yes" },
    { ...evidence(), graders: [{ type: "exact_match", passed: true }] },
  ];
...
test("compiler snapshots and validates raw config before runner mutation", async () => {
  const original = baseInput().config;
  const mutableConfig = { ...original };
  let runs = 0;
  const remainingBudgets = [];
  const result = await compile(baseInput({
    config: mutableConfig,
    async runner({ maxCostUsd }) {
      runs++;
      remainingBudgets.push(maxCostUsd);
      mutableConfig.maxSearchCostUsd = 0;
      mutableConfig.qualityRetention = 0;
      return evidence();
    },
  }));
  assert.equal(result.status, "locked");
  assert.ok(runs > 1);
  assert.equal(remainingBudgets[0], original.maxSearchCostUsd);
  assert.ok(remainingBudgets.every((value, index) =>
    index === 0 || value <= remainingBudgets[index - 1]));
  assert.ok(remainingBudgets.at(-1) < remainingBudgets[0]);
  assert.throws(() => defineBuild({
    entry: "./agent.ts",
    evals: "./evals.ts",
    maxSearchCostUsd: Number.POSITIVE_INFINITY,
  }), /maxSearchCostUsd must be positive/);
  await assert.rejects(compile(baseInput({
    config: { ...original, qualityRetention: "0" },
  })), /invalid build config shape/);
});

test("final paid run cannot mint a lock after crossing actual search cap", async () => {
  const baseline = plan("baseline");
  const result = await compile(baseInput({
    baselinePlan: baseline,
    candidates: [{ plan: baseline, estimated_cost_usd_per_run: 0 }],
    ...
    config: defineBuild({
      entry: "./agent.ts",
      evals: "./evals.ts",
      maxSearchCostUsd: 0.025,
    }),
    runner: async () => evidence({ cost: 0.03 }),
  }));
  assert.equal(result.status, "search_budget_exceeded");
  assert.equal(result.completed_runs, 1);
  assert.equal(result.actual_cost_usd, 0.03);
  assert.equal(result.lock, undefined);
});

test("same-lineage eval ids count as one retention family", async () => {
  const baseline = plan("baseline");
...
test("supplied candidates fail closed on Context IR, recovery, and policy before runner", async () => {
  const baseline = plan("baseline");
  let runs = 0;
  const unsafe = transformedPlan("unsafe-segment");
  unsafe.segment_routes[0].segment_id = "missing-s4-segment";
  await assert.rejects(compile(baseInput({
    baselinePlan: baseline,
    candidates: [
      { plan: baseline, estimated_cost_usd_per_run: 0 },
      { plan: unsafe, estimated_cost_usd_per_run: 0 },
    ],
    runner: async () => { runs++; return evidence(); },
  })), /cave_candidate_segment_route_unsafe/);

  const badRecovery = {
    ...baseline,
    plan_id: "bad-recovery",
    recovery: { namespace: "test", tools: ["cave_retrieve"] },
  };
  await assert.rejects(compile(baseInput({
    baselinePlan: baseline,
    candidates: [
      { plan: baseline, estimated_cost_usd_per_run: 0 },
      { plan: badRecovery, estimated_cost_usd_per_run: 0 },
    ],
    runner: async () => { runs++; return evidence(); },
  })), /cave_candidate_recovery_tools_invalid/);

  const denied = await compile(baseInput({
    baselinePlan: baseline,
    candidates: [{ plan: baseline, estimated_cost_usd_per_run: 0 }],
    config: defineBuild({
      entry: "./agent.ts",
      evals: "./evals.ts",
      deniedModels: [baseline.model],
    }),
    runner: async () => { runs++; return evidence(); },
  }));
  assert.equal(denied.status, "incomplete_evidence");
  assert.equal(runs, 0);
});

test("compiler recomputes candidate ceiling and run cost from catalog token buckets", async () => {
  const baseline = plan("baseline");
  const result = await compile(baseInput({
    baselinePlan: baseline,
    candidates: [{ plan: baseline, estimated_cost_usd_per_run: 0 }],
    runner: async () => ({
      ...evidence({ cost: 0.02 }),
      // Both caller-owned price fields understate this run. Compiler must ignore
      // them and price authoritative provider/model/token buckets itself.
      price_basis: "unpriced",
      catalog_cost_usd: 0,
    }),
  }));
  assert.equal(result.status, "locked");
  assert.ok(result.estimated_ceiling_usd > 0);
  assert.equal(result.actual_cost_usd, 0.2);
  assert.equal(result.baseline_catalog_cost_usd_per_task, 0.02);
  assert.equal(result.selected_catalog_cost_usd_per_task, 0.02);
...
test("compiler rejects provider or model identity drift against candidate plan", async () => {
  ...
  for (const identity of [
    ...
    const result = await compile(baseInput({
      ...
      evals: [fixture()],
      seeds: [1],
      runner: async () => ({ ...evidence({ cost: 0.01 }), ...identity }),
    }));
    assert.equal(result.status, "incomplete_evidence");
    assert.equal(result.reason, "provider/model identity drift against candidate plan");
    assert.equal(result.lock, undefined);
  }
});
...
test("warm cache cost regression rejects cheaper cold candidate", async () => {
  ...
  const input = baseInput({
    baselinePlan: baseline,
    evals: [dynamicFixture("one"), dynamicFixture("two")],
    candidates: [
    ...
    runner: async ({ plan: selected, seed }) => selected.plan_id === baseline.plan_id
      ? evidence({ cost: 0.01, cachePrefix: hex("c"), graderType: "tool_called" })
      : evidence({
        cost: seed === 1 ? 0.001 : 0.011,
        cachePrefix: hex("d"),
        cacheRead: seed === 1 ? 0 : 10,
        cacheWrite: seed === 1 ? 10 : 0,
        graderType: "tool_called",
      }),
  });
  const result = await compile(input);
  assert.equal(result.status, "locked");
...
test("five independent zero-cache transform runs cannot lock", async () => {
  ...
  const result = await compile(baseInput({
    ...
    runner: async ({ plan: selected }) =>
      evidence({
        cost: selected.plan_id === baseline.plan_id ? 0.02 : 0.001,
        graderType: "tool_called",
      }),
  }));
  assert.equal(result.status, "locked");
...
test("search ceiling uses worst catalog input class and rejects unknown models", () => {
  assert.equal(catalogSearchCeiling("anthropic/claude-haiku-4-5", 1_000_000, 0), 1.25);
