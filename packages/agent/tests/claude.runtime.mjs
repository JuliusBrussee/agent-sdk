...
import { CLAUDE_CODE_VERSION } from "../dist/runtime-identity.js";
...
async function lockedBuild() {
  // The Claude lane rejects this private option before it inspects a build.
  // Keeping the fixture opaque isolates that authorization invariant from the
  // compiler's separate fail-closed price-evidence gate.
  return Object.freeze({ schema_version: 1, build_id: hex("f") });
}
...
test("Claude reasoning policy matches pinned model capabilities before spend", async () => {
  ...
  await assert.rejects(
    ...
    /cave_claude_model_not_runtime_eligible/,
  );
  assert.equal(unknownCapture.calls, 0);
});

test("Claude exact model lifecycle gate also runs when reasoning is off", async () => {
  const fixtures = [
    ["discovered", { lifecycle: "discovered", messagesAPI: "supported" }],
    ["retired", { lifecycle: "retired", messagesAPI: "supported" }],
    ["unsupported", { lifecycle: "reviewed", messagesAPI: "unsupported" }],
    ["unknown", { lifecycle: "reviewed", messagesAPI: "unknown" }],
  ];
  for (const [state, facts] of fixtures) {
    const capture = { calls: 0 };
    const model = `claude-${state}-fixture`;
    const rejected = agent({
      ...definition(),
      id: `claude-off-${state}-model`,
      model: `anthropic/${model}`,
      reasoning: "off",
    });
    await assert.rejects(
      runClaudeAgentInternal(rejected, "must not spend", {
        ensureRuntime: false,
        queryFn: queryFn(capture, 5, model),
        modelFactsFn: () => ({
          ...facts,
    ...
    assert.equal(capture.calls, 0, `${state} model reached the provider query`);
  }
});
