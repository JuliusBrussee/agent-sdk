...
test("package lifecycle builds executable framework CLI before packing", async () => {
  ...
  for (const [directory, upstream, version] of [
    ...
    assert.equal(adapter.peerDependencies["@caveman-ai/agent"], pkg.version);
    assert.equal(adapter.peerDependencies[upstream], version);
  }
  assert.equal(
...
test("generated eval contains no admission gate and unknown graders fail closed", () => {
  ...
  assert.throws(() => defineEval({
    id: "empty-contains",
    input: "x",
    quality: [{ type: "contains", fragments: [] }],
  }), /contains grader needs non-empty fragments/);
  assert.throws(() => defineEval({
    id: "empty-tool-called",
    input: "x",
...
test("compiler candidate cost cap reserves declared plan context, not full model window", async () => {
  const defined = agent({
    id: "compiler-plan-cost-cap",
    instructions: "Answer exactly: ok",
    model: auto(),
    reasoning: "off",
    sandbox: "fixture",
  });
  ...
  const plan = {
    schema_version: 1,
    plan_id: "compiler-plan-cost-cap",
    ...
    recovery: { namespace: "compiler-plan-cost-cap", tools: [] },
    fallbacks: { unknown: "original", transform_error: "original", not_smaller: "original" },
  };
  const planCeiling = catalogSearchCeiling(`${model.provider}/${model.id}`, 1_000, 64);
  const fullWindowCeiling = catalogSearchCeiling(
    `${model.provider}/${model.id}`,
    model.contextWindow,
    64,
  );
  assert.ok(planCeiling !== undefined && fullWindowCeiling !== undefined);
  assert.ok(planCeiling < fullWindowCeiling);
  let calls = 0;
  const result = await runAgentInternal(defined, "go", {
    ...
    streamFn(selected) {
      calls += 1;
      return manualTextStream(selected, "ok");
    },
  });
  assert.equal(result.text, "ok");
...
test("sandbox dependency grants cover hoisted packages without granting root node_modules", () => {
  const roots = sandboxDependencyReadRoots();
  const typeboxManifest = findPackageJSON("typebox", import.meta.url);
  assert.notEqual(typeboxManifest, undefined);
  const typeboxRoot = dirname(typeboxManifest);
  const pnpm = typeboxRoot.indexOf("/node_modules/.pnpm/");
  const expected = pnpm >= 0
    ? typeboxRoot.slice(0, pnpm + "/node_modules/.pnpm".length)
    : typeboxRoot;
