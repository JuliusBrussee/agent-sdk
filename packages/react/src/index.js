 ...
 * bundle, so this hook has no token option and no absolute-origin option: it
 * calls a same-origin path in YOUR app, and your route forwards it with the
 * credential attached. The README shows the six-line proxy.
 *
...
import { INITIAL_STATE, reduceAgentEvent } from "./events.js";

export { INITIAL_STATE, reduceAgentEvent } from "./events.js";
...
export function useAgent({ api }) {
  ...
  /**
   * Stop watching. This does NOT cancel the run: the server has no cancel
   * endpoint, so the agent keeps working and keeps spending. Reattach with
   * `watch(runId)`.
   */
