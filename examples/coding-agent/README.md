...
An interactive coding agent built on
[`@caveman-ai/coding-agent`](../../packages/coding-agent).
It reads, greps, runs shell commands, creates and edits files in one workspace, and it
...
```bash
npm --prefix packages/agent run build  # build core package used by coding-agent
export ANTHROPIC_API_KEY=...           # or OPENAI_API_KEY / GEMINI_API_KEY
node --preserve-symlinks examples/coding-agent/agent.mjs
```
...
- Tool output is capped **before** compression (24 KB for `read_file`/`bash`,
  ...
  engine present. Bash preserves the tail. Large captured results expose a
  process-local `read_tool_output` handle for bounded paging or literal search;
  repeating the expensive command is unnecessary.
- Coding turns use deterministic loop, no-progress, and fan-out breakers.
  Pass `budget: { maxUsd: ... }` or `budget: { maxTokens: ... }` for a hard
  per-turn cap enforced before provider calls.
